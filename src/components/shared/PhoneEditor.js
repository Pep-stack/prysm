...
export default function PhoneEditor({ value = '', onChange, onSave, onCancel }) {
  const [phoneNumber, setPhoneNumber] = useState('');

  useEffect(() => {
    if (value) {
      // Extract phone number from tel: URL or use directly
  ...
  const handleSave = () => {
    if (phoneNumber.trim()) {
      ...
      const telUrl = `tel:${phoneNumber.trim()}`;
      onChange(telUrl);
      onSave(telUrl);
  ...
  return (
    ...
    >
      {/* Phone Header with logo and text */}
      <div className="flex items-center justify-between p-6 pb-4" style={{ backgroundColor: '#000000' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold text-lg">Phone Number</h3>
            <p className="text-gray-400 text-sm">Add your phone number</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 pt-4">
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-xs mt-2">We'll create a direct call link for easy contact</p>
        </div>
