...
export default function TikTokEditor({ value = '', onChange, onSave, onCancel }) {
  ...
  const handleSave = () => {
    if (username.trim()) {
      // Convert username to TikTok URL
      const tiktokUrl = `https://www.tiktok.com/@${username.trim()}`;
    ...
    >
      ...
      <div className="flex items-center justify-between p-6 pb-4" style={{ backgroundColor: '#000000' }}>
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-full" style={{ backgroundColor: '#fe2c55' }}>
            <FaTiktok className="text-white text-xl" />
          </div>
          <div>
            <h3 className="text-white font-semibold text-lg">TikTok Profile</h3>
            <p className="text-gray-400 text-sm">Connect your TikTok account</p>
          </div>
        </div>
      </div>
