...
export default function FacebookEditor({ value = '', onChange, onSave, onCancel }) {
  ...
  useEffect(() => {
    if (value) {
      ...
      if (facebookMatch) {
        setUsername(facebookMatch[1]);
      } else {
        // If it's just a username, use it directly
        setUsername(value.replace('https://facebook.com/', '').replace('https://www.facebook.com/', ''));
      }
    } else {
      setUsername('');
    }
  ...
  const handleSave = () => {
    if (username.trim()) {
      // Convert username to Facebook URL
      const facebookUrl = `https://facebook.com/${username.trim()}`;
  ...
  return (
    ...
    >
      {/* Facebook Header with logo and text */}
      <div className="flex items-center justify-between p-6 pb-4" style={{ backgroundColor: '#000000' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold text-lg">Facebook Profile</h3>
            <p className="text-gray-400 text-sm">Connect your Facebook account</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 pt-4">
        <div className="mb-6">
          ...
          <div className="relative">
            <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-sm">facebook.com/</span>
            <input
              type="text"
              value={username}
