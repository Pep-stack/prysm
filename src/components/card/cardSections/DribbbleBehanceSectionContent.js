...
// Simple clean design icon with dynamic size and color
const DesignIcon = ({ color = 'white', size = '24px' }) => (
  <svg 
...
export default function DribbbleBehanceSectionContent({ profile, styles, isCompact = false }) {
  ...
  if (hasValidUrls) {
    ...
    return (
      <div style={{ display: 'inline-flex', gap: '8px' }}>
        {validUrls.map((url, index) => (
          ...
          >
            <DesignIcon color={iconColor} size={iconSize} />
          </a>
        ))}
      </div>
    );
    ...
    return (
      <a 
        ...
        title="Add Dribbble/Behance URL(s)"
        onMouseEnter={(e) => e.target.style.opacity = '0.7'}
